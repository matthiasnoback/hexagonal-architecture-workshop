...
final class Application implements ApplicationInterface
{
    ...
    public function rsvpForMeetup(RsvpForMeetup $command): void
    {
        $statement = $this->connection
            ...
            ->setParameter('meetupId', (string) $command->meetupId())
            ->execute();
        ...
        $rsvp = Rsvp::create((string) $command->meetupId(), $command->userId());
        $this->rsvpRepository->save($rsvp);

        $this->eventDispatcher->dispatch(
            new UserHasRsvpd($rsvp->meetupId(), $rsvp->userId(), $rsvp->rsvpId())
        );
    }
}
