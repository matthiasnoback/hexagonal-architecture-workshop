...
final class OutboxRelayCommand extends Command
{
    ...
    {
        $record = $this->connection->fetchAssociative('SELECT * FROM outbox WHERE wasPublished = 0 ORDER BY messageId LIMIT 1');
        if ($record === false) {
            return;
        }

        $decodedData = json_decode(Mapping::getString($record, 'messageData'), true);
        ...
        $this->connection->update('outbox', ['wasPublished' => 1], ['messageId' => Mapping::getInt($record, 'messageId')]);
    }
}
